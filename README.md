...
On Windows, connectivity monitoring initializes COM on a dedicated observer thread and queries Network List Manager; the thread owns a drop guard that calls `CoUninitialize`. If COM activation is unavailable, the observer falls back to a bounded five-second reachability probe against the configured provider host. Non-Windows builds use the no-op portability path. Power resume is edge-mapped to the coordinator and never performs network work in the adapter.
