...
pub fn calculate(i: PaceInput) -> Option<Pace> {
    ...
    if duration <= 0 || elapsed < 0 || elapsed > duration || !i.used_percent.is_finite() {
        return None;
    }
...
mod tests {
    ...
    fn computes_expected_marker_and_status() {
        ...
        .unwrap();
        assert_eq!(p.expected_percent, 45.0);
        assert_eq!(p.delta_percent, 15.0);
